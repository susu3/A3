...
{   
    ...
    if (asprintf(&msg, "You are an expert in %s protocol fuzz testing. Your task is to generate **valid initial request messages** for fuzzing a binary protocol.\n"
        "\n"
        "### Input Data:\n"
        "- **Protocol Specification Document**: The authoritative reference describing the protocol's structure, message formats, field definitions, and constraints.\n"
        "  * Use this document to understand the correct message structure\n"
        "  * Follow all field definitions, byte orders, and constraints specified\n"
        "  * Verify correctness of all generated messages against this specification\n"
        "\n"
        "### Your Task:\n"
        "Generate **exactly 20 diverse and valid request messages** suitable as fuzzing seeds. Each message must be a complete, well-formed protocol request that a real client would send.\n"
        "\n"
        "### Diversity Requirements (Generate at least 2-3 messages for each category):\n"
        "1. **Message Type Diversity**: Cover all major message/command types defined in the protocol\n"
        "2. **Boundary Values**: Include minimum, maximum, and typical values for numeric fields\n"
        "3. **Length Variations**: For variable-length fields, test minimum length, maximum length, and medium length\n"
        "4. **Function Code Coverage**: Include different function codes, command types, or operation modes\n"
        "5. **Data Pattern Diversity**: Use different data patterns (all zeros, all ones, sequential, random)\n"
        "6. **Edge Cases**: Include edge cases like empty optional fields, maximum array sizes, boundary conditions\n"
        "\n"
        "### Correctness Requirements (Critical - Violations will cause failures):\n"
        "1. **Byte Order**: Strictly follow the endianness (big-endian or little-endian)\n"
        "2. **Field Structure**: Follow the exact field order, types, and sizes\n"
        "3. **Length Fields**: Calculate ALL length fields accurately based on actual message content\n"
        "4. **Value Constraints**: Respect all value ranges and constraints\n"
        "5. **Field Dependencies**: Respect all conditional fields and dependencies\n"
        "6. **Checksums/CRCs**: If the protocol requires checksums or CRCs, calculate them correctly\n"
        "7. **Default Values**: Use default values from the grammar when specified\n"
        "8. **Required Fields**: Include ALL required fields in the correct order\n"
        "\n"
        "### Common Errors to Avoid:\n"
        "- Wrong byte order (mixing big-endian and little-endian)\n"
        "- Incorrect length field calculations (off-by-one errors, not counting headers)\n"
        "- Missing required fields or including them in wrong order\n"
        "- Invalid field values outside allowed ranges\n"
        "- Incorrect padding or alignment\n"
        "- Wrong checksum calculations\n"
        "\n"
        "### Output Format (Strict):\n"
        "- Output ONLY the <sequence></sequence> blocks, nothing else\n"
        "- Each message must be wrapped with <sequence></sequence> tags\n"
        "- Inside each sequence: hex-encoded bytes with exactly ONE space after every 4 hex characters (2 bytes)\n"
        "- Use uppercase hex digits (A-F)\n"
        "- Do NOT include: field names, comments, explanations, descriptions, or any text outside <sequence> tags\n"
        "- Each <sequence> tag must be on its own line\n"
        "\n"
        "### Examples of Valid Output Format:\n"
        "For the MODBUS protocol: <sequence>0000 0000 0008 FF16 0004 00F2 0025</sequence>\n"
        "For the IEC104 protocol: <sequence>6804 0700 0000 6804 4300 0000 6804 1300 0000</sequence>\n"
        "%s"
        "\n"
        "====================\n"
        ...
        "\n"
        "### Task:\n"
        "Generate exactly 20 diverse, valid, and well-formed request messages:\n"
        "- Cover diverse message types and values\n"
        "- Output ONLY <sequence></sequence> blocks\n"
        "\n"
...
char *construct_prompt_for_seeds_sequence(char *protocol_name, char **final_msg, char *rfc_path){
    ...
    char *content = NULL;
    if (asprintf(&content, "You are an expert in %s protocol fuzz testing. Your task is to generate **valid request message sequences** for fuzzing an industrial binary protocol.\n"
    "\n"
    ...
    "\n"
    "### Available Inputs:\n"
    "**Protocol specification**: Authoritative source in natural language.\n"
    "\n"
    "### Requirements:\n"
    "\n"
    ...
    *final_msg = content;
    ...
    ck_free(escaped_content);
    free(rfc_file_content);  // Free RFC content before returning
